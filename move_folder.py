...
def load_config():
    ...
    if file_path:  # 如果选择了文件
        ...
        if 'PATHS' in config:  # 检查是否有PATHS节
            config_dir = os.path.dirname(file_path)  # 获取配置文件所在目录
            source_folder_abs = config.get('PATHS','source_folder', fallback='')  # 获取源文件夹绝对路径
            if source_folder_abs:  # 如果路径存在
                # 将绝对路径转换为相对于配置文件的路径
                source_folder = os.path.join(config_dir, os.path.relpath(source_folder_abs, config_dir))
            else:
                source_folder = ''  # 否则设为空字符串
...
# 创建"文件"菜单
file_menu = tk.Menu(menu_bar, tearoff=0)  # 创建子菜单
menu_bar.add_cascade(label="文件", menu=file_menu)  # 添加级联菜单
file_menu.add_command(label="保存配置", command=save_config)  # 添加保存配置菜单项
file_menu.add_command(label="加载配置", command=load_config)  # 添加加载配置菜单项

# 创建"关于"菜单
about_menu = tk.Menu(menu_bar, tearoff=0)  # 创建子菜单
menu_bar.add_cascade(label="关于", menu=about_menu)  # 添加级联菜单
# 使用lambda表达式创建匿名函数作为回调
about_menu.add_command(label="关于本程序", command=lambda: tk.messagebox.showinfo("关于", "这是一个移动文件夹的小程序"))
