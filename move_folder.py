...
def load_config():
    ...
    if file_path:
        ...
        if 'PATHS' in config:
            ...
            source_folder_abs = config.get('PATHS','source_folder', fallback='')
            if source_folder_abs:
...
file_menu.add_command(label="保存配置", command=save_config)
file_menu.add_command(label="加载配置", command=load_config)
